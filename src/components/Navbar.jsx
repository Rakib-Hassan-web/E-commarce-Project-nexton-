      ...
      <div className="nav_row flex justify-between items-center">
        ...
        <Form onSubmit={searchHandler}
          ...
          < RiSearch2Line className='text-2xl text-praymary  srach_icon' id='srach_icon' onClick={()=>setvalue(!value)}
            />
            <input onChange={(e)=>setsearch_Inp(e.target.value)} className='navinp w-full border-none outline-none
            text-[14px] font-normal text-praymary font-praymary '
        ...
        <div id='CART' className={`fixed top-0 h-screen w-full left-0 flex justify-end bg-[#0000009b] z-10
          duration-[.3s] ${cart? 'hidden' : 'visible' }` }>
      ...
      <div key={item.id} className='flex items-center  pl-25 justify-between px-20 mt-10'>
        ...
        <div className='flex items-center gap-5'>
          <div className='w-[80px] h-[80px]   mb-3'>
            <img src={item.images[0]} alt="product image" className='rounded-[20px]' />
          </div>
